...
class Category(models.Model):  # Таблица категория которая наследует models.Model
    name = models.CharField('Наименование', max_length=100)  # varchar.Нам потребуется только имя категории

    class Meta:
        verbose_name = ("Категории")  # человекочитаемое имя объекта
        verbose_name_plural = ("Категории")  # человекочитаемое множественное имя для Категорий
...
class TodoList(models.Model):
    ...
    created = models.DateField('Дата создания', default=timezone.now().strftime("%Y-%m-%d")) # дата создания
    due_date = models.DateField('Дата окончания', default=timezone.now().strftime("%Y-%m-%d")) #до какой даты нужно было сделать дело
